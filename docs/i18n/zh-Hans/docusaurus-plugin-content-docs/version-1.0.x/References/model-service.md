...
### 1. ModelService
位于`rock/sdk/model/service.py`，负责服务的生命周期管理:
- 启动模型服务(本地或代理模式)
- 监控代理进程
- 停止模型服务

### 2. ModelClient
位于`rock/sdk/model/client.py`，提供与模型服务通信的客户端功能:
- 推送模型的响应到系统
- 从系统中获取新的请求
- 管理请求/响应的索引以保持顺序

### 3. 配置
位于`rock/sdk/model/server/config.py`，定义了服务的配置参数:
- 服务主机和端口 (默认8080)
- 日志文件路径
- 请求和响应的标记字符串
- 轮询间隔等参数
...
### 代理API
当前代理API (`/v1/chat/completions`) 尚未实现

## CLI命令

模型服务提供了一个CLI命令集，可以通过 `rock model-service` 访问:
